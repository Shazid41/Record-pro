...
import { Student, SortField, SortOrder } from './types.ts';
import StudentForm from './components/StudentForm.tsx';
import StudentTable from './components/StudentTable.tsx';
import Dashboard from './components/Dashboard.tsx';
import { getAcademicInsights, generateSampleStudents } from './services/gemini.ts';
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-slate-200 p-8 sticky top-0 h-screen shrink-0 z-50">
        ...
        <nav className="flex-1 space-y-3">
          <NavItem view="dashboard" label="Overview" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>} />
          <NavItem view="records" label="Students" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>} />
          <NavItem view="ai" label="AI Insights" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>} />
          <NavItem view="settings" label="Settings" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>} />
        </nav>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                  ...
                  <div className="flex gap-3 w-full sm:w-auto">
                    <button onClick={handleAddSampleData} className="flex-1 sm:flex-none text-sm font-bold text-slate-600 px-5 py-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all">Import Sample</button>
                    <button onClick={handleAiAnalysis} className="flex-1 sm:flex-none text-sm font-bold text-white px-5 py-3 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">AI Audit</button>
                  </div>
                </div>
                <StudentTable students={filteredAndSortedStudents} onDelete={handleDelete} onEdit={handleEdit} sortField={sortField} sortOrder={sortOrder} onSort={handleSort} />
              </div>
            )}
            ...
            {currentView === 'ai' && (
              <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out max-w-4xl mx-auto">
                 <div className="bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 rounded-[2.5rem] p-8 lg:p-12 text-white shadow-[0_25px_50px_-12px_rgba(79,70,229,0.3)] mb-10 relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl transition-all duration-700"></div>
                    <div className="relative z-10">
                      <h1 className="text-4xl lg:text-5xl font-black mb-6 tracking-tightest leading-tight">AI Academic <br className="hidden sm:block" /> Advisor</h1>
                      <p className="text-indigo-100 text-lg lg:text-xl mb-10 max-w-xl opacity-90 leading-relaxed">Deep analysis of student datasets using Gemini 3 Flash.</p>
                      <button onClick={handleAiAnalysis} disabled={isAnalyzing || students.length === 0} className="bg-white text-indigo-900 px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:scale-105 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 shadow-2xl">
                        {isAnalyzing ? <div className="animate-spin h-5 w-5 border-2 border-indigo-900 border-t-transparent rounded-full" /> : 'Analyze Records Now'}
                      </button>
                    </div>
                 </div>
                 {aiInsights ? (
                   <div className="bg-white rounded-[2.5rem] p-8 lg:p-12 border border-slate-100 shadow-xl shadow-slate-200/50 animate-in zoom-in duration-500">
                      <div className="prose prose-slate max-w-none">
                        {aiInsights.split('\n').map((line, i) => <p key={i} className="text-slate-600 text-lg leading-relaxed mb-4">{line}</p>)}
                      </div>
                   </div>
                 ) : (
                   <div className="text-center py-24 bg-white rounded-[2.5rem] border-4 border-dashed border-slate-100">
                      <div className="text-indigo-100 mb-6 flex justify-center"><svg className="w-24 h-24" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg></div>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out max-w-2xl">
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 divide-y divide-slate-50 overflow-hidden">
                  <div className="p-8 lg:p-10">
                    ...
                    <p className="text-slate-500 font-medium mb-6 leading-relaxed">Your data is synced locally and prepared for Netlify edge functions.</p>
                    <div className="flex flex-wrap gap-4">
                      <button onClick={handleAddSampleData} className="px-6 py-3.5 bg-indigo-50 text-indigo-700 rounded-2xl font-bold hover:bg-indigo-100 transition-all text-sm">Generate Mock Data</button>
                      <button onClick={() => { if(confirm("Wipe all local records?")) { setStudents([]); localStorage.removeItem('student_records'); } }} className="px-6 py-3.5 bg-rose-50 text-rose-600 rounded-2xl font-bold hover:bg-rose-100 transition-all text-sm">Reset Local Database</button>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {isFormOpen && <StudentForm onSubmit={handleAddOrUpdate} initialData={editingStudent} onCancel={() => { setIsFormOpen(false); setEditingStudent(null); }} />}
    </div>
  );
};
...
const StatCard = ({ label, value, color }: { label: string, value: string | number, color: string }) => {
  ...
  return (
    <div className={`p-5 lg:p-7 rounded-[2rem] border bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl duration-300 group`}>
      ...
      <div className="flex items-end justify-between">
        ...
        <div className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${colorMap[color] || colorMap.indigo}`}>Live</div>
      </div>
    </div>
  );
};
...
const MobileNavItem = ({ active, onClick, icon }: { active: boolean, onClick: () => void, icon: React.ReactNode }) => (
  <button onClick={onClick} className={`p-3.5 rounded-2xl transition-all duration-300 ${active ? 'bg-indigo-600 text-white shadow-xl scale-110' : 'text-slate-400'}`}>{icon}</button>
);
